...
public class RecognizerTask implements Runnable {
	...
	//JAM - A creator with context, so we can get properly stored data files. 
	public RecognizerTask(Context context) {
		
		//JAM - Copy data files to storage
		Utilities utils = new Utilities(context);
		String base_dir = context.getFilesDir().getAbsolutePath();
		utils.CopyAssetDirectoryToStorage("speech_data/hmm/en_US/hub4wsj_sc_8k", base_dir + "/hmm");
		utils.CopyAssetDirectoryToStorage("speech_data/lm/en_US_one2five", base_dir + "/lm");
			
		pocketsphinx.setLogfile("/sdcard/Android/data/edu.cmu.pocketsphinx/pocketsphinx.log");
//		pocketsphinx.setLogfile(base_dir + "/pocketsphinx.log");
		Config c = new Config();
